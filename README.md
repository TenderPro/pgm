...
при запуске команд будут переопределены на те, которые содержаться в файле .conf. Т.е. если в .conf не содержится DB_PASS, то его значение будет взято из fidm.yml и пароль будет задан в виде строки равной 'SET_DB_PASS_HERE_OR_IN_fidm.yml'

Для того, чтобы в Makefile-е подключится к конкретному контейнеру pgrest необходимо использовать имя в формате pgrest_$${APP_SITE}_www, где APP_SITE - это имя сайта.
